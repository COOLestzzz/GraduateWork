...
import com.smart.vo.MessageDto;

@ServerEndpoint("/websocketTest")
public class WebSocketUtil {
	private static int onlineCount = 0;
	//存放所有登录用户的Map集合，键：每个用户的唯一标识（用户名）
	private static Map<String,WebSocketUtil> webSocketMap = new HashMap<String,WebSocketUtil>();
	...
	public static void setHttpSession(HttpSession httpSession){
		WebSocketUtil.httpSession=httpSession;
	}
	/**
	 * 连接建立成功调用的方法
	 * @param session
	...
	public void sendOnlineCount(String message){
		for (Entry<String,WebSocketUtil> entry  : webSocketMap.entrySet()) {
			try {
		...
		if(messageStr.indexOf("@")!=-1){
			...
			for (Entry<String,WebSocketUtil> entry  : webSocketMap.entrySet()) {
				...
				if(targetname.equals(entry.getKey())){
					try {
						for (Entry<String,WebSocketUtil> entry1  : webSocketMap.entrySet()) {
